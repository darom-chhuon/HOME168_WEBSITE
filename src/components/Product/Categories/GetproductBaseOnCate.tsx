...
export default function ProductListingBaseOnCat() {
  interface Product {
    id: number;
    name: string;
    price: number;
  ...
  const fetchProduct = async () => {
    ...
    try {
      ...
      const response  = await axios.get(
        `${baseUrl}/${apiPrefix}/items/latest?store_id=1&category_id=${slug}&limit=17&offset=1`,
        {
          headers: {
            moduleId: 1,
            zoneId: "[1]"
      ...
      console.log("Responding", response.data.products);
    } catch (error) {
